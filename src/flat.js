...
function lowerToUpper(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    // 小写
      ...
      unicodeMap.a <= el.charCodeAt(0) &&
      el.charCodeAt(0) <= unicodeMap.z
    ) {
      el = String.fromCharCode(el.charCodeAt(0) - diff);
    } else {
      continue;
...
function InstanceOf(L, R) {
  if (
    ...
    return false;
  let lPrototype = Object.getPrototypeOf(L),
    rPrototype = R.prototype;
...
// console.log('twoSum([2, 7, 11, 15], 9): ', twoSum24([2, 7, 11, 15], 9));
...
//     //   let remain = target - arr[j];
//     //   if (map.has(remain)) {
//     //     return [map.get(remain), j];
//     //   }
//     // }
//   }
...
let s2 = '[object Object]';
const regex = /\[object (\w+)\]/;
let r2 = s2.match(regex);
console.log(r2[1]);
