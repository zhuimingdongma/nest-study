...
export class ChannelService {
  ...
  async view(channelViewDto: ChannelViewDto) {
    try {
      ...
      .where("channel.name like :name", {name: `%${name ?? ''}%`}).andWhere("channel.system like :system", {system: `%${system ?? ''}%`})
  ...
  async delete(channelDelDto: ChannelDelDto) {
    try {
      ...
      const {affected} = await this.channelRepository.delete({id: channelId})
      if (affected === 0) return new HttpException("删除失败", HttpStatus.UNPROCESSABLE_ENTITY)
      else return "删除成功"
    }
    catch (err) {
      return new HttpException(err, HttpStatus.FAILED_DEPENDENCY)
  ...
  async update(channelUpdateDto: ChannelUpdateDto) {
    try {
      ...
      repository.id = id;
      if (gameId) {
        foundGame = await this.gameListRepository.findOne({where: {id: gameId}})
        if (new Tools().isNull(foundGame)) return new NotFoundException("未找到该游戏")
        repository.gameList = foundGame!;
      }
      const {affected} = await this.channelRepository.update(id, repository)
      if (affected === 1) return "更新成功"
      else return new HttpException("更新失败", HttpStatus.UNPROCESSABLE_ENTITY)
    }
    catch(err) {  
      return new HttpException(err, HttpStatus.FAILED_DEPENDENCY)
    }
