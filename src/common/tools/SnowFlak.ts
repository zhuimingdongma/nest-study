export class SnowFlake {
  ...
  constructor(machine_id: number = 1, data_center_id: number = 1) {
    if (machine_id >= this.MAX_MACHINE_NUM || machine_id < 0) {
      ...
      data_center_id >= this.MAX_DATA_CENTER_NUM ||
      data_center_id < 0
    ) {
  ...
  nextId() {
    ...
    if (timeStamp < this.lastTimestamp) {
      throw new Error(
        '系统时钟回调 拒绝生成id' + (timeStamp - this.lastTimestamp),
      );
    }
    if (timeStamp === this.lastTimestamp) {
    ...
    const timeStampPos =
      BigInt(timeStamp - this.START_TIME) << BigInt(this.TIMESTAMP_LEFT);
    const dataCenterPos =
      BigInt(this.data_center_id) << BigInt(this.DATA_CENTER_LEFT);
